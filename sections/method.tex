...
\end{equation}
where $m_{ij}$ is a calibrated surface sample expressed in $S_i$ and
$\alpha_{ij}\in[0,1]$ is its effective association reliability. Records are
matched by acquisition time rather than trajectory row index. Thus the online
state is cell-indexed, while the correction backend receives bounded
vertex-indexed records without retaining the range stream. Its contract is
\begin{equation}
...
\end{equation}
With optional frontend quality $\eta_{ij}\in[0,1]$ (set to one when absent),
...
\label{eq:target}
\end{equation}
Controls failing the fixed support or dispersion criteria are discarded. The
...
vertex $a$. Their initial proxy displacement and target are
\begin{equation}
...
Let $\Omega_{\rm fb}$ contain bounded unsupported interpolation holes that are
connected to $\Omega_{\rm tr}$ but not incident to an open mesh boundary. Its
finite fallback weight is
\begin{equation}
w_{\rm fb}(x)=
\begin{cases}
1, & \|x-q^+\|\le0.5s,\\
...
\end{equation}
The warped normal inherits the source-face orientation, giving the fallback
side sign
\begin{equation}
