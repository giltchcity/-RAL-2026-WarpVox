...
\quad |\mathcal O_j|\le n_v,
\label{eq:resolved_records}
\end{equation}
where $m_{ij}$ is expressed in $S_i$ and $\alpha_{ij}\in[0,1]$ is its effective
...
$\mathcal J_a=\{j:\pi(j)=a\}$ collect accepted controls assigned to proxy
vertex $a$. For $\mathcal J_a\neq\emptyset$, their initial proxy displacement
and target are
\begin{equation}
\bar d_a=\frac{1}{|\mathcal J_a|}\sum_{j\in\mathcal J_a}d_j,
\qquad
p_a^{(0)}=p_a^-+\bar d_a.
...
Let $E_P=E(F_P)$ be the proxy edges. WarpVox performs a single event-level
proxy ARAP optimization~\cite{ARAP_modeling:2007,
...
\end{equation}
Define the active output domain $\Omega^+=\Omega_{\rm tr}\cup\Omega_{\rm fb}$.
On this domain, WarpVox sets
\begin{equation}
\Phi^+(x)=
\begin{cases}
...
The accumulated frame count $N$ does not appear: bounded support replaces
history-dependent replay, while the remaining work follows the active surface,
proxy, and output volume.