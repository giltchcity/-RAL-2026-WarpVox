...
WarpVox uses four stages: (i)~associate bounded sensor-frame evidence with the pre-correction surface, (ii)~compute robust targets from corrected poses, (iii)~propagate sparse displacements through one ARAP solve~\cite{ARAP_modeling:2007} on a QEM proxy, and (iv)~reconstruct a TSDF whose zero set follows the warped surface while support and confidence follow the source volume. The last step matters because a surface-only conversion would mark unsupported space as observed. Once the records are maintained by the mapper, correction scales with active surface and volume rather than accumulated frames.
